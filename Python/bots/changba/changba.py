...
def get_work_list(userid, worknum, latestid = ''):
    worknum = int(worknum)
    page_sum = worknum / 20
    if worknum % 20 != 0:
        page_sum += 1
...
def download_list(work_list, prefix):
    for song in work_list:
        ...
        try:
            ...
            time.sleep(2)
            address = parse_song_address(song['enworkid'])
        subprocess.call('wget %s -O "%s/%s.mp3"' % (address, prefix, filename), shell = True, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
